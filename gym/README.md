...
All end-to-end scenarios belong directly in `gym/tests`. Unit tests for the host runner or terminal helper belong beside their source in `packages/gym/sources`.
...
This builds `rig-gym:local` and then runs every file in `gym/tests`. Docker caches image layers, and the test process reuses the completed image build across gym instances.
...
1. Add a descriptively named test under `gym/tests`.
