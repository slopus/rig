...
export const claudeGrepTool = defineTool({
  ...
  arguments: Type.Object({
    ...
    glob: Type.Optional(
      Type.String({ description: 'Glob pattern to filter files (e.g. "*.js", "*.{ts,tsx}") - maps to rg --glob' }),
    ),
    ...
    "-B": Type.Optional(Type.Number({ description: 'Number of lines to show before each match (rg -B). Requires output_mode: "content", ignored otherwise.' })),
    "-A": Type.Optional(Type.Number({ description: 'Number of lines to show after each match (rg -A). Requires output_mode: "content", ignored otherwise.' })),
    "-C": Type.Optional(Type.Number({ description: "Alias for context." })),
    context: Type.Optional(Type.Number({ description: 'Number of lines to show before and after each match (rg -C). Requires output_mode: "content", ignored otherwise.' })),
    "-n": Type.Optional(Type.Boolean({ description: 'Show line numbers in output (rg -n). Requires output_mode: "content", ignored otherwise. Defaults to true.' })),
