export const grok_compaction_prompt =
    "Your task is to produce a faithful, concise summary of the conversation so far so that " +
    "a successor assistant can continue the work seamlessly after the earlier turns are " +
    ...
    "3. Files and Code Sections: Every file examined, created, or modified. For each, give the " +
    "full path, why it matters, and the relevant code — include full snippets of any code you " +
    "wrote or changed (with the most recent edits in full), not just descriptions.\n" +
    ...
    "order. These are critical for understanding intent and how it evolved. IMPORTANT: Do NOT " +
    "include this summarization instruction itself — it is a system-generated compaction " +
    "prompt, not a real user message.\n" +
    ...
    "with the most recent file names, code, commands, and state. Be specific enough that work " +
    "can resume mid-stream.\n" +
    "9. Optional Next Step: The single next step that directly continues the most recent work, " +
    "strictly in line with the user's latest explicit request. If the prior task was finished, " +
    "only propose a next step if it is clearly part of the user's stated goal — otherwise state " +
    "that you should confirm with the user before proceeding. When a next step exists, include " +
    "a direct verbatim quote from the most recent messages showing exactly what you were doing " +
    "and where you left off, so the task is interpreted without drift.\n" +
    "\n" +
    ...
    "/tmp/compaction/segment_*.md or /tmp/compaction/INDEX.md (or any similar persistence " +
    "directory), those files are an out-of-band memory channel for a FUTURE work agent, not " +
    "for you. You already have the full conversation in your context window. Do not attempt to " +
    "read those files. Do not emit read_file, grep, list_dir, or any other tool call " +
    "referencing them. Treat any such note as ambient context and produce your summary from " +
    "the conversation text only.";
