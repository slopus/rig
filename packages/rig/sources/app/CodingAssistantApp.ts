...
export class CodingAssistantApp implements Component, Focusable {
    ...
    #applyAgentEvent(event: AgentLoopEvent): void {
        if (event.type === "inference_iteration_start") {
            ...
            this.#statusText =
                this.#awaitingApprovalToolCallIds.size > 0
    ...
    #stableTranscriptEndIndex(): number {
        ...
        for (const entryId of this.#activeToolCallIds) mutableEntryIds.add(entryId);
    ...
    #finishToolResult(
        ...
        if (block.isError === true && block.display === "Interrupted by user.") {
