...
export function toPiContext(context: Context): PiContext {
    return {
        ...
        ...(context.tools !== undefined ? { tools: context.tools.map(toPiTool) } : {}),
    };
}
...
function toPiToolResultContent(content: ToolResultContent): PiTextContent | PiImageContent {
...
function toPiAssistantContent(
    ...
    if (content.type === "thinking") {
        return {
            ...
            ...(content.encrypted !== undefined ? { thinkingSignature: content.encrypted } : {}),
...
function fromPiAssistantContent(content: PiAssistantMessage["content"][number]): AssistantContent {
    if (content.type === "text") {
